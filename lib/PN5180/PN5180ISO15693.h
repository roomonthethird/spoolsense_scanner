...
public:
  ...
  ISO15693ErrorCode readMultipleBlocks(uint8_t *uid, uint8_t startBlock, uint8_t numBlocks, uint8_t *blockData, uint8_t blockSize);
...
public:
  bool setupRF();
  // ESP32 compatibility: changed from __FlashStringHelper* to const char*
  const char *strerror(ISO15693ErrorCode errcode);
